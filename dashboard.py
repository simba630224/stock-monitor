...
# ==========================================
# 2. 核心抓取與計算邏輯 (修正 009815 的 yfinance 代號規格)
# ==========================================
def get_yf_ticker_tw(ticker):
    ticker = str(ticker).strip()
    # 針對債券 (以B結尾) 或 6位數新創/海外型/櫃買 ETF (如 009815) 自動導向櫃買中心 .TWO
    if re.match(r'^\d+B$', ticker) or (ticker.isdigit() and len(ticker) == 6):
        return f"{ticker}.TWO"
    return f"{ticker}.TW"
...
def process_technical_analysis(sym, name):
    try:
        ...
        elif macd_d < macds_d and pmacd_d >= pmacd_d and (macd_d != 0 or macds_d != 0):
