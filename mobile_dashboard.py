...
def process_technical_analysis(sym, name, market):
    try:
        ...
        if has_enough_weekly:
            if k_w > d_w and pk_w <= pd_w and k_w > 0: alerts.append("週KD低檔金叉" if k_w < 30 else "週KD金叉")
            elif k_w < d_w and pk_w >= pd_w and d_w > 0: alerts.append("週KD高檔死叉" if k_w > 70 else "週KD死叉")
            
            if macd_w > macds_w and pmacd_w <= pmacds_w and (macd_w != 0 or macds_w != 0): alerts.append("週MACD零下金叉" if macd_w < 0 else "週MACD金叉")
            elif macd_w < macds_w and pmacd_w >= pmacds_w and (macd_w != 0 or macds_w != 0): alerts.append("週MACD零上死叉" if macd_w > 0 else "週MACD死叉")
        ...
        has_reduce = any(x in a for a in alerts for x in ["20日高點回落"])
...
with tab2:
    ...
    with st.expander("💡 狀態警示名詞定義說明", expanded=True):
        ...
        * **綜合買賣評級**：系統依據技術指標自動判斷的交易建議。
            * **🚀 買進**：出現週 KD 或週 MACD 黃金交叉，屬於中長線翻多訊號。
            * **🛑 賣出**：出現週 KD 或週 MACD 死亡交叉，或自近一年高點回落達 15% 等翻空訊號。
            * **⚠️ 減碼**：自 20 日高點回落達 10% 等短線轉弱訊號。
    ...
    if selected_name and selected_name != "暫無可繪圖標的":
        ...
        if df_chart is not None:
            ...
            if 'K_d' in df_plot.columns:
                fig_tech.add_trace(go.Scatter(x=df_plot.index, y=df_plot['K_d'], line=dict(color='blue', width=1.5), name='K值'), row=2, col=
